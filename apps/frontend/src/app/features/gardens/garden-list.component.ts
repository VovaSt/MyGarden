...
export class GardenListComponent implements OnInit {
  ...
  readonly form = this.formBuilder.nonNullable.group({
    ...
    widthMeters: [10, [Validators.required, Validators.min(0.01)]],
    heightMeters: [10, [Validators.required, Validators.min(0.01)]],
    description: [''],
  });
  ...
  load(): void {
    this.api.getAll().subscribe({
      next: (gardens) => this.gardens.set(gardens),
      error: () => this.error.set('Не вдалося завантажити сади. Перевірте backend.'),
    });
  }
  ...
  create(): void {
    ...
    this.api.create({
      ...
      next: (garden) => {
        this.gardens.update((items) => [...items, garden]);
        this.form.reset({ name: '', widthMeters: 10, heightMeters: 10, description: '' });
      },
      error: () => this.error.set('Не вдалося створити сад.'),
